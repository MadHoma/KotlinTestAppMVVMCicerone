package com.test.kotlin.domain.base
...
abstract class UseCase<out T> where T : Any {
    ...
    abstract class RxSingle<T> : UseCase<Single<T>>() {
        ...
        fun execute(onSuccess: (T) -> Unit, onError: (Throwable) -> Unit, onLoading: ((Boolean) -> Unit)? = null) =
            disposables.add(build()
                .subscribeOn(Schedulers.computation())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe { onLoading?.invoke(true) }
                .doOnEvent { _, _ -> onLoading?.invoke(false) }
